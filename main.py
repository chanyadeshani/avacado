...
def handle_button_click(n_clicks, questions_dropdown_value, school_dropdown_value, course_group_dropdown_value,
                        subject_dropdown_value, slider_value, passcode_value):
        ...
        if "submit-button" in prop_id:
            if n_clicks is not None and n_clicks > 0 and questions_dropdown_value and passcode_value == "nss23@UOG":
                ...
                api_response = make_api_request(
                    questions_dropdown_value, slider_value, school_dropdown_value, course_group_dropdown_value,
                    subject_dropdown_value)
                ...
                if api_response is not None:
                    ...
                    if df.size > 0:
                        ...
                        df_pvhi = df[(df["valence_score"] > 0) & (df["arousal_score"] > 0)].sort_values(
                            by=['arousal_score'], ascending=False)
                        df_pvli = df[(df["valence_score"] > 0) & (df["arousal_score"] < 0)].sort_values(
                            by=['arousal_score'], ascending=False)
                        df_nvli = df[(df["valence_score"] < 0) & (df["arousal_score"] < 0)].sort_values(
                            by=['arousal_score'], ascending=False)
                        df_nvhi = df[(df["valence_score"] < 0) & (df["arousal_score"] > 0)].sort_values(
                            by=['arousal_score'], ascending=False)
    ...
    return blank_fig(), blank_fig(), [], [], [], [], [], [], [], [], [], [], [], [], not bool(
        questions_dropdown_value), True, True, True, True, True, True
        ...
        html.Div(
            ...
            children=[
                ...
                html.Div(
                    ...
                    children=[
                        dcc.Dropdown(
                            options=["How good are teaching staff at explaining things?",
                                     ...
                                     "How well has your course developed your knowledge and skills that you think you will need for yourfuture?",
